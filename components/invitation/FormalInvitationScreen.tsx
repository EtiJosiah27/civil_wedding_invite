...
export function FormalInvitationScreen({ active, onContinue }: { active: boolean; onContinue: () => void }) {
  ...
  return <section className={`screen content-screen ${active ? 'is-active' : ''}`} aria-hidden={!active} inert={!active}><OrnateFrame><article className="formal-content">
    ...
    <div className="invitation-actions"><GoldButton className="gift-button" onClick={() => { setShowTransferNote(false); setGiftsOpen(true); }}>Gifts</GoldButton><GoldButton onClick={onContinue}>RSVP&nbsp; →</GoldButton></div>
    {giftsOpen && <div className="gift-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setGiftsOpen(false); }}><dialog className="gift-modal" open aria-labelledby="gift-modal-title">
      ...
      <div className="gift-options"><a className="gold-button" href="https://www.amazon.ca/wedding/guest-view/3PMNPVZPB52AZ" target="_blank" rel="noreferrer">Amazon Registry&nbsp; →</a><GoldButton onClick={() => setShowTransferNote(true)}>Interac e-Transfer</GoldButton></div>
      {showTransferNote && <output className="transfer-note">E-transfer recipient details will be added soon.</output>}
    </dialog></div>}
